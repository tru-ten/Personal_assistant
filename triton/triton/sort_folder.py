...
class UnsortedFile:
    ...
    def normalized_name(self) -> str:
        return normalize(self.stem)

    def move(self, target_dir: pathlib.Path, new_stem: str = None):
        ...
        if new_stem:
            new_name = target_dir.joinpath(
                self.__path.with_stem(new_stem).name)
        else:
    ...
    def sort(self):
        for path in self.to_iter:
            if path.is_file():
                ...
                cat = file.category
                nn = file.normalized_name()
                file.move(self.paths[cat], nn)
